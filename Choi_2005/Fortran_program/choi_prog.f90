...
! In particular, this program is set up to model the data found in Choi, Balsam and Horvitz's
! 2005 paper "Extended Habit Training Reduces Dopamine Mediation of Appetitive Response Expression"
...
integer, parameter :: max_response_time = 10000
...
open(10,file = 'input\choi_params.dat')
...
do trial = 1, num_trials
		...
		if (mod(trial,50) == 1) open(40, file = 'output\ActivationsT'//trim(trial_string)//'.dat')
		...
		! If the rat sticks its head in the box during the next time period it receives a
		! reward.  
