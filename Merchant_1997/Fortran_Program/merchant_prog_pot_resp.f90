...
integer, parameter :: num_past = 10

integer, parameter :: max_response_time = 1600

integer, parameter :: feedback_time = 500
integer, parameter :: three_factor_time = 2500

integer, parameter :: theta_num = 11, alpha_num = 4, beta_num = 2

integer :: wait_time ! wait_time varies between 1500 and 4500 by trial

real :: caud_base, glob_base, thal_base, premot_base
real :: caud_start, caud_range

real :: pot_thresh, pot_mult, premot_thresh, caud_thresh, response_thresh

integer :: t, trial, time_points, i,j, num_correct, two_factor_time
...
real :: PotCA, PotCB, PotGA, PotGB, PotTA, PotTB, PotMA, PotMB
...
num_correct = 0
	...
	do trial = 1, num_trials
		...
		! Finally, the activity in the thalamus causes activity in a premotor area
			...
			PotMA = pot_mult*PotMA + ActiveMA(t)
			...
			PotMB = pot_mult*PotMB + ActiveMB(t)
		...
		! Now we see if we have a response, and which response we have
			...
			if (PotMA >= response_thresh) A_flag = .true.

			if (PotMB >= response_thresh) B_flag = .true.
	
			if (A_flag .or. B_flag) then 
				...
				goto 500 ! Now that we have a response, we terminate the visual stimulus
...
500 		continue
		...
		! Before moving to the final portion of the trial, we turn off the
		! inputs from visual cortex
...
time_points = wait_time + max_response_time + feedback_time

write(*,*) 'Number of Trials =', num_trials
write(*,*) 'Milliseconds per Trial =', time_points
...
new_caud_act = cap(old_act + theta(1)*vis_input*(1-old_act) - theta(2)*(old_act - caud_base) - &
		theta(3)*old_comp_act + noise*caud_noise_std*old_act*(1-old_act))
...
new_glob_act = cap(old_act - theta(4)*caud_input*old_act - theta(5)*(old_act-glob_base))
...
new_thal_act = cap(old_act - theta(6)*glob_input*old_act - theta(7)*(old_act-thal_base))
...
new_premot_act = cap(old_act + (theta(8)*vis_input+theta(9)*thal_input)*(1-old_act)-theta(10)*(old_act-premot_base) - &
			theta(11)*old_comp_act + noise*premot_noise_std*old_act*(1-old_act))
...
threefactor = theta + alpha(1)*Pos(caud_sum-caud_thresh)*vis_sum*dope_plus*(1-theta) &
	...
	Pos(caud_thresh-caud_sum)*vis_sum*theta - alpha(4)*theta*decay_factor
